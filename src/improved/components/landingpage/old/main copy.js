...
function Compile(component, config = {}) {
  ...
  function Generate() {
    ...
    const { template, operations } = compile(
    ...
    observedAttributes.map((attr) => {
      componentConfiguration.obbservedAttributeTypes[attr] =
        typeof Instance[attr];
      try {
    ...
    return class extends HTMLElement {
      static get observedAttributes() {
        return componentConfiguration.obbservedAttributes;
      }
...
const PreCompileWebComponent = Compile(WebComponent, {
  ...
  @if(isShowing){
    ...
    <section >
      <h1>The custom element is showing.</h1>
...
const PreCompileCustomElement = Compile(CustomElement, {
  ...
  template: `<div class="border border-gray-100 p-4  m-4 mx-auto">
  <div class="bg-{{color}}-100 p-8 " >
  <header>
    <h1 >{{newTitle}}</h1>
    <p >{{color}}</p>
    <p >{{title}}</p>
