export const extractLookedPaths = function (
  _scope,
  expression,
  _preffix = "this"
) {
  ...
  function extract(scope, expressions = [], preffix = _preffix, paths = []) {
    ...
    for (let key of rootKeys) {
      for (let parsedExpression of parsedExpressions) {
        let idx = Math.max(
          -1,
          parsedExpression.indexOf(preffix + "." + key),
          parsedExpression.indexOf(preffix + "[")
        );
  ...
  if (
    parsedExpressions[1] &&
    parsedExpressions[1].indexOf(_preffix + ".") > -1
  ) {
